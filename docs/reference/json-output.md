...
The daemon's `status` method carries no version or protocol field
(Studio request 1/7); the hub's install lifecycle emits no NDJSON progress
records (request 6); `import`, `remove`, `clean` — the write commands — keep
their prompts and progress bars. Each lands as a row of
`studio-engine-requests.md` when it does.
