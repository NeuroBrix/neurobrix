...
## A — blocks a line of the catalogue (21), in the order of what they convert

| debt | catalogue line(s) it holds | what closes it | state on 2026-09-13 |
|---|---|---|---|
| D-DEEPSTACK-ZERO-EXTENT | Qwen3-VL-30B-A3B-Thinking, Qwen3-Omni-30B-A3B-Instruct — both engines refuse at the first bind since 09-10 | the build toolchain declares min 0 for an optional input; two re-traces (57 GB each), rebuild, re-upload | filed 09-13; the 4 cells fail identically in rerun 2 and in rerun 3 (09-14 08:24, `46479ae`: `Symbol s6 (seq_len) = 0, below the minimum extent the container declares (1)`); **09-16: the build-toolchain rule landed** (the DeepStack stimulus declares its embeds' visual count min 0, from the vendor config's declaration, never a name; test seen red) — the two 57 GB re-traces, rebuild and re-upload are queued behind the merge of the Mac branch; until they land the two lines still refuse |
| D-DECLARED-MOE-AS-EXECUTED-VIEW | Ming-Lite-Omni-1.5 triton (`moe_fused None.data_ptr`), Qwen3-Omni thinker sized 5.2 GB / executed 57 GB | one as-executed view per component computed at solve, read by every sizer; register 54's loader fix is the first half (`46479ae`) | filed 09-13; rerun 3 on `46479ae` (09-14 08:24): DeepSeek-Coder-V2-Lite native and deepseek-moe native PASS (the OOM at weight load is gone — register 54's loader fix proven by run); Ming triton passes the loader and stops at the next wall: the launcher refuses `embedding_kernel` a device address the allocator never handed out (`0x7fba35100000`) — filed 09-14 as D-MING-UNTRACKED-ADDRESS-AT-EMBEDDING; **09-16 named and fixed at the source**: the address sat inside a Triton loader arena FREED before the first kernel — the weight-key re-binding rebuilt the dict without its `_arenas` entry (their last reference); the re-binding now keeps every key it does not name, and the declared-MoE fusion's second load JOINS its arenas to the first's instead of replacing them (the free site named by the malloc trace's new free rows; three tests seen red); **CLOSED by run 12:52** (`--triton --mode text` rc=0 on two cards) |
| D-MOCHI-CUDA-700-AT-MM → D-PRISM-MOCHI-VAE-ACTIVATION-UNDERESTIMATED | Mochi-1-preview (CUDA 700 at its first matmul, then at add, then at group_norm) | the CUDA 700 is CLOSED at the kernels: three int32 offset wraps past 2^31 elements (registers 58, `90fefd4` `7ee3d7c` `aa60c5c`), named by `--triton-sequential` + launch blocking after two sanitizer runs measured nothing; the run now reaches the VAE decoder and OOMs at `silu::26` (8.75 GB asked, 26 GB live, 32 GB card) where Prism planned 3.2 GB of activations and no tiling — the estimator, filed 09-14 | kernel side closed 09-14 09:05; **09-16, diagnosed — two causes, neither the estimator's arithmetic**: (1) the container is STALE (traced 07-07): at `aten._unsafe_view::1` H and W are literals 28/44 while today's view rule keeps the expression factors (three build-toolchain tests pass on the current tracer) — the fix is a re-trace of mochi (38 GB), queued behind the merge; (2) the estimate of an uncalibrated component ran at fp16 while the conservative precision path stores fp32 — the estimate now sizes ACTIVATIONS at the precision contract's own decision (weights keep their dtype; a first wiring that touched the component map doubled thirty weight bills in the 56-container sweep and was caught by that measurement), landed once the three plans it moves are budgeted under the model that executes them (budget cell behind the battery) |
| D-MING-EMBED-LENGTHS-DECLARED-AND-UNREFERENCED | Ming-Lite-Omni-1.5, Qwen3-VL and Qwen3-Omni with a media input of a size other than the trace's — **suspected, not established** | a run at another image or audio length, which is the only thing that can answer it; then, if it refuses, the build side promoting those dimensions to symbols the graph REFERENCES | filed 2026-09-16 by the symbol-chain census. What is MEASURED: `image_embeds` dim 0 (12) and `audio_embeds` dim 0 (14) in Ming's `model.model`, and `deepstack_visual_embeds.0` dim 0 (12) in Qwen3-VL and Qwen3-Omni, are declared and referenced by NO operation, while every other symbol of those graphs is referenced hundreds of times. What is NOT established, and an earlier version of this line claimed it: that any operator FREEZES them. The census first named two freeze sites in Ming and both dissolved on reading — `aten.view::1` records `[s0, s1, 24, 128]` and carries its symbols (24 is the head count, 3072 = 24x128, not 12x2), `aten.split_with_sizes::0` records `[16, 4, 4]`, a GQA split, not 14+2. MiniCPM's equivalents (`vision_hidden_states` 7, `audio_hidden_states` 11) reach only `masked_scatter`, which takes no shape argument at all. So the graph may well be free at those dimensions; a declaration nothing names is a question, not a verdict, and the run is what settles it |
| D-WARM-COMPILED-BMM-ADDRESS-REFUSED-UNDER-TORCH-2.14 | the WARM serving path of vlm, multimodal and image (GLM-4.1V-9B-Thinking, Janus-Pro-7B, Sana-1600M-MultiLing) on the candidate stack — and with it the STACK SWITCH | the launcher refuses `aten.bmm::0` a device address the allocator never handed out, on the second (warm) request only; the cold cells of the same models pass on the same stack | filed 2026-09-17 by the battery on the candidate stack. **Attributed by measurement, not inference**: same tree, same cell, only the interpreter changing, three repetitions per arm alternating — torch 2.14.0+cu126 / Triton 3.8.0 fails 3/3 with one address refusal each, torch 2.5.1+cu121 / Triton 3.6.0 passes 3/3 with none. The allocator's segment mode is refuted as the cause (`PYTORCH_CUDA_ALLOC_CONF=expandable_segments` False and True both fail identically). Same message class as D-MING-UNTRACKED-ADDRESS-AT-EMBEDDING, closed 09-16 in the triton path; this one is the COMPILED path. **Narrowed the same hour**: the TRITON warm path passes on BOTH stacks (image-triton, rc=0, zero refusals), and the cold cells pass on both — so it is the COMPILED warm path under the new stack and nothing else. The compiled path hands the launcher a torch tensor's address; the triton path's tensors are NBXTensors the allocator issued itself. `nbx/campaigns/2026_09_16_converge/BATTERY_T38_VERDICT.md` |
...
| D-CONTAINER-ENGINE-COMPATIBILITY-UNDECLARED | sixteen matrix cells that no measurement could have produced: the before tree cannot load containers built after it, and nothing declares which engine a container needs | `nbx_version` bumps at a loader-contract change (Forge), the engine declares the range it loads (`info --json`), `NBXContainer.load` refuses by name, a paired harness reads both before spending an arm — R18 is touched (an existing field acquires meaning): owner's decision | filed 09-16; **re-posed 09-16** on 46479ae vs the merged trunk: GLM-4.1V, Janus, MiniCPM-o IDENTICAL on the Triton engine; Qwen3-VL/Omni wait for their retrace, Ming triton has no before arm (its arena defect) — three of the six models measured, the rule itself still the owner's decision |
...
| the §4.2 screen cell (A40) | B | the production wiring is the live screen's oracle (register 45-47); the sm_86 cell itself is owed by whoever has the card — **2026-09-16: no A40 on this rack (four V100s), the cell is not run here; it stays owed to the machine that carries the card, said rather than masked** |
