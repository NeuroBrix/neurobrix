...
|---|---|---|---|
| D-DEEPSTACK-ZERO-EXTENT | Qwen3-VL-30B-A3B-Thinking, Qwen3-Omni-30B-A3B-Instruct — both engines refuse at the first bind since 09-10 | the build toolchain declares min 0 for an optional input; two re-traces (57 GB each), rebuild, re-upload | filed 09-13; the 4 cells fail identically in rerun 2 |
| D-DECLARED-MOE-AS-EXECUTED-VIEW | Ming-Lite-Omni-1.5 triton (`moe_fused None.data_ptr`), Qwen3-Omni thinker sized 5.2 GB / executed 57 GB | one as-executed view per component computed at solve, read by every sizer; register 54's loader fix is the first half (`46479ae`) | filed 09-13; rerun 3 on `46479ae` is the proof by run, pending |
| D-MOCHI-CUDA-700-AT-MM | Mochi-1-preview (FAILED rc=-9, then illegal memory access at its first matmul) | the sanitizer names the faulting kernel; fix at the kernel or the trace | under `compute-sanitizer` on cards 2+3 since 21:26, rig quiet |
...
| D-PRISM-2x16-PIPELINE-OVERFILL | GLM-4.1V-9B-Thinking triton on `--gpu 0,1` (22 GB planned on 2×16) | a per-card share that fits, or a refusal by name before the first malloc | filed 09-07 |
...
| *(new, 09-13)* register 56 — memory-class coverage | every entry proven on one memory class is served to no other: the 32 GB cards sweep until covered; 1 530 rig-proven entries serve no card until re-proven | certification of the 16 GB-proven keys on a 32 GB card (cards 2/3, behind the chain), re-proof of the 1 530 on a 16 GB card | door landed `d1def45`; re-proof on cards 0/1 DONE 09-14 01:39 (card 1: 367, card 0: 1 092; checkpointer commits `c7d6706`…`8c1b94b`; directory 8 498 entries, 8 498 on 16 GB, 17 on 32 GB, 0 unknown; every catalogue shape reads `16 GB N/N · 32 GB 0/N` until cards 2/3 certify); the 16→32 sample (50 keys) read 13/50 different configs and the 32 GB card 1.175× slower — UNADJUDICATED card-vs-day until the control on card 0 reads (register 56, measured paragraph) |
