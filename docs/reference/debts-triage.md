...
| D-MING-EMBED-LENGTHS-FROZEN | Ming-Lite-Omni-1.5 with an image or an audio input of a size other than the trace's | the build side promotes `image_embeds` dim 0 and `audio_embeds` dim 0 to symbols the graph REFERENCES; today both are declared and named by none of the component's 18 672 operations | filed 2026-09-16 by the symbol-chain census (`tools/where_the_symbol_chain_breaks.py`): s8 (seq_len=12, `input::image_embeds::dim_0`) and s9 (seq_len=14, `input::audio_embeds::dim_0`) are referenced 0 times while every other symbol of that graph is referenced 224 to 1 066 times — so the number of image tokens and of audio tokens is the trace's. Not observed as a failure yet: the text line closed by run on 09-16 and the multimodal request has not been run at another media size |
