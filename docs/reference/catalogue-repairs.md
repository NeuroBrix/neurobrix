...
| proof by run | **compiled (the oracle): PASSED 2026-09-13 11:35** — 9 frames at 448×448, range 0–255, mean inter-frame difference 3.91, 101 862 bytes, at the container's default guidance (`validation_outputs/proof_by_run_Wan2.2-I2V-A14B-Diffusers_20260913_1127/compiled.mp4`). **triton: renders at `--cfg 1.0`** (9 frames, range 0–255, difference 1.01, byte-identical on the pre-merge and merged engines) and **does not fit at the default guidance**: with batched CFG the triton live set reaches 31 327 MB on the single 32 GB card Prism chose (plan: 27 254 weights + 1 315 activations + 1 428 overhead = 30.0 GB of 32.5) and the first attention asks 1.77 GB of fp32 scores with 792 MB free — measured identically on both engines, rig empty. Prism refuses the two-card alternatives (`NBX_FORCE_STRATEGY=component_placement` and `=weight_sharding`: *"cannot fit the model on the given hardware profile"*) on a rig with 96 GB in four cards. A Prism budget finding, not a container defect: DETTE `D-PRISM-WAN22-TRITON-ONE-CARD`. On the way here the proof found and fixed a real triton defect (register 48: the consumed-weight filter dropped the embedding). |
...
| re-trace (2) | **five attempts on 2026-09-13, each refused for a cause the previous one had not reached**: (1) 22:56 — no `model_index.json` (arrangement rebuilt); (2) 11:28 — `_diffusers_version: 0.0.0` made every component fail at vendor setup, and the tokenizer directories were enumerated as components (set to 0.33.1, tokenizers left out of the list); (3) 12:41 — `MMDiTModel`/`AutoencoderKLCausal3D` declared under an `opensora` library were looked up in transformers (declared under `diffusers` so the worker falls through to its custom loaders); and, pinned to one card, MMDiT died on an illegal memory access at `.to(dtype)` — 11B parameters constructed in fp32 in a single 31 GB VMM slab (traces are never GPU-masked: unpinned since); (4) 12:44 — the T5 shards are `.bin` and the vendored transformers refuses `torch.load` below torch 2.6: safetensors shards written beside them (19.6 GB, contiguous tensors); (5) **12:50 → 4/4 graphs**: transformer 5089 ops (= June's), text_encoder 1594 (June 1597), text_encoder_2 490, **vae 237 (June: 11 017)**. Every step is written beside the weights in `NEUROBRIX_LAYOUT.md`. |
| rebuild (2) | **12:55:51 → rc=0 after 682 s**, built on the export (the root filesystem's 54 GB could not hold the ~85 GB peak; a superseded, published 14 GB staged build was removed under `nbx/builds/README.md`'s rule); graphs 4/4 from the fresh cache |
| regression gate (2) | text_encoder 1.000×, text_encoder_2 1.000×, transformer 1.000×, **vae 0.959× — refused, then opened with `--allow-shrink` on a measurement**: the VAE's weights index is identical (248 tensors, same bytes each, bfloat16 both); what shrank is the graph, 11 017 → 237 ops (19.5 → 6.5 MB of JSON). Whether the short graph decodes is the proof by run's question, not the gate's. |
| re-upload · install · proof by run | upload started 13:08:54 through the internal entry point (`replace`, adaptive pacing); install follows; the proof runs in the final chain after the tail's cells leave the rig |
