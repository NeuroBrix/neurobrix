...
| `genmo/Mochi-1-preview` | video | 38.2 | **RENDERS on triton at 9 frames (1 step, 118 s) — the CUDA 700 was three int32 index wraps past 2^31 elements, a defect of EVERY kernel for ANY model whose tensor exceeds two billion elements (the next family to meet it will not be called Mochi), fixed for the class at the kernels; at its default 84 frames the VAE decoder OOMs where Prism planned 3.2 GB (estimator debt)** | 0† | 0† | not measured | 0 shapes met | transformer `seq_len`@256 (weight-extent); transformer `seq_len`@256 (weight-extent) | `D-MOCHI-CUDA-700-AT-MM` | measured |
