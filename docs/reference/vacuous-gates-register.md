...
* **the census — a family, not an accident** the question the instance raises is how widely the gate has been passing things it should not. Asked of all 56 local containers, 182 component graphs, resolved at three bindings. **Only ONE component's estimate exceeds the whole rack** — the instance itself — so the harm has landed once. But the SIGNATURE is everywhere in one family: restricting the test to ops whose output extents are structurally constrained (convolution, slice, cat, pad, group/layer norm — never a view or reshape, where a product in one slot is legitimate), **22 components across 12 containers** carry an output dim resolving to more than 100× its traced value and above 10 000. Those twelve are **every video container in the zoo** — Allegro, Allegro-TI2V, CogVideoX-2b, CogVideoX-5b-I2V, Open-Sora-v2, SANA-Video, all four Wan, mochi — plus `chatterbox` with two dims. The 5-D video graphs carry this by construction; it has produced a visible failure in exactly one place because only there does it land on the estimator's peak. Sample worst cases: `native_layer_norm` dim1 15600 → 4 194 304, `cat` dim1 17776 → 4 194 530, `constant_pad_nd` dim4 178 → 32 770.
* **and what the census is NOT** a proof that each of those 22 is a defect. It is a signature count at chosen bindings against a chosen threshold, and both are choices. What it establishes is the shape of the answer: this is endemic to one family, so the repair is one build-side chantier and not twelve model fixes. A first, looser form of this test — literal-on-one-side against expression-on-the-other — flagged 63 components including TinyLlama, whisper and swinir, which all run; it was discarded rather than shipped.
