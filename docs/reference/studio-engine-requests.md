...
| 2 | **Decoded text deltas in the live stream** | The streaming callback emits `{"step", "n", "token": token_id, "done"}` (`serving/server.py:338-342`); the full text arrives with the terminal response (`:386`) | The stream carries the text the engine's own tokenizer decodes for each delta; both engines (compiled, triton) — the tokenizer runner is engine-internal already (Tokenizer Engine) | R30 (mode universality): one seam in each generator, same event shape. R34: decoding stays inside the engine; Studio never ships a tokenizer. **Landed 2026-09-16:** every stream event of a text family carries `text` (the delta to append, decoded by the engine's tokenizer; a split multi-byte character is held until complete) and `rewind` (characters to take back when a later token rewrites emitted text); the family's own declaration (output format) decides, not its name (`serving/text_stream.py`, `serving/server.py::stream_listener`, both engines through the one listener). |
...
| 5 | **JSON for hub queries, inventory, removal; NDJSON for long-running CLI progress; diagnostics to stderr** | `registry` prints tables and progress bars for a person; the import path already has licence and gated-access requirements the plan asks Studio to preserve | `--json` on the registry commands, one line per progress event on stdout, everything else on stderr; the licence prompt becomes an explicit parameter, never a silently passed flag | Data-driven output (Output Dispatch Engine's discipline applied to the CLI's own records). **Landed 2026-09-16 (read half):** `--json` on every read command, one record on stdout, human lines on stderr, schema versioned per command and gated by a parse test. **Second half landed the same day:** `import --json` is NDJSON — one event per phase with the real byte counts, human lines on stderr, every refusal one terminal `error`; the licence is an explicit parameter under `--json` (`--accept-license`, else a refusal naming it — never a prompt); `remove --json` is one record (`docs/reference/json-output.md`). `clean` still prompts. |
| 6 | **Download / extraction / installation lifecycle stays in the engine** — real byte counts, phases, cleanup of incomplete work, a model visible only after success | The importer resumes an interrupted download (D-IMPORT-RESUMABLE-DOWNLOAD, closed 2026-09-13) and installs in phases; the phases are logged, not emitted as records | The same lifecycle emitting one record per phase (item 5's NDJSON), the "visible only after success" rule stated by the installer rather than assumed | Hub doctrine (five layers): the installed set is the engine's, Studio reads it through the engine. **Landed 2026-09-16:** the lifecycle emits `info`, `license`, `download` (bytes, total), `downloaded`, `extracting`, `installed`, `done` / `error`; the model is visible under its name only after extraction succeeded — it extracts into `<name>.installing`, renamed in one motion, and a `*.installing` directory is never listed; a failed extraction leaves nothing under either name (`registry.py::installing_path`, `is_installed_model_dir`). |
