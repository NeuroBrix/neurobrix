# Community

NeuroBrix is an open-source project and welcomes contributions from the community.

## Get Involved

- **[Contributing](contributing.md)** — How to contribute code, docs, and bug reports
- **[Hardware Integration](hardware-integration.md)** — Program for adding new GPU support
- **[Model Registry](model-registry.md)** — Program for contributing new models

## Links

- [GitLab Repository](https://gitlab.com/neurobrix/Neurobrix)
- [Issue Tracker](https://gitlab.com/neurobrix/Neurobrix/-/issues)
- [PyPI Package](https://pypi.org/project/neurobrix/)
- [Model Hub](https://neurobrix.es/models)
