...
class RuntimeExecutor:
    ...
    def _execute_component(
        ...
        # Layer 9 — DC-AE VAE decoder tiling for memory-bound runtime
        # spatial sizes (Sana 4Kpx and beyond). Activates ONLY for
        # `comp_name == "vae"` in `phase == "post_loop"`, only when the
        # runtime image-space spatial dim exceeds the per-architecture
        # threshold defined in the vendor YAML
        # (`config/vendors/<vendor>/<arch>.yml::vae_decoder.tile_above_image_dim`).
        # Sana 1024 / PixArt 1024 stay below the threshold and run the
        # single-shot path unchanged — Layer 6.3 contract preserved
        # (tiling is opt-in, never global).
        if (output is None and comp_name == "vae" and phase == "post_loop"
                and handler is not None):
            tiled = self._maybe_dc_ae_tiled_decode(
                comp_name, comp_inputs, executor, handler)
    ...
    def _maybe_dc_ae_tiled_decode(
        self, comp_name: str, comp_inputs: Dict[str, Any],
        executor: Any, handler: Any,
    ) -> Optional[Any]:
        """Run the DC-AE VAE decoder via the tiled path when the runtime
        spatial size exceeds the per-architecture threshold.

        Returns the decoded sample (4D tensor) when tiling fired, or
        `None` to fall through to the standard execution path.

        Activation rule:
          - VAE handler must report a `spatial_compression_ratio`
            (DC-AE: 32; KL VAEs: 8). Without it, fall through.
          - Latent input must be a 4D tensor (image diffusion). Video
            5D paths are out of scope.
          - Runtime image-space dim (= latent_h * ratio) must exceed
            `vae_decoder.tile_above_image_dim` from the vendor YAML.

        Bit-exact equivalence vs single-shot is ensured by dispatching
        the SAME executor.run() for each tile — only the spatial
        extent of the input differs. The diffusers blend at seams
        introduces a controlled overlap-blend (linear interpolation
        across the tile_sample_min - tile_sample_stride border, NVlabs
        defaults: 128 px = 1024 - 896).
        """
        # Locate the latent input. The handler exposes a private finder.
        find_latent = getattr(handler, "_find_latent_key", None)
        if find_latent is None:
            return None
        latent_key = find_latent(comp_inputs)
        if latent_key is None:
            return None
        latent = comp_inputs[latent_key]
        if not hasattr(latent, "shape") or not hasattr(latent, "dim"):
            return None
        if latent.dim() != 4:
            return None  # video / non-image latents skip this path

        # Spatial compression ratio from the VAE handler (already
        # data-driven via block_out_channels / vae_scale_factor in the
        # component's profile.json).
        try:
            ratio = int(handler.get_latent_scale())
        except Exception:
            return None
        if ratio <= 0:
            return None

        # DC-AE canonical compression ratio is 32. Non-DC-AE VAEs
        # (KL VAE at ratio=8 used by PixArt, Sana 1024, SDXL etc.) fall
        # through to single-shot path immediately. Layer 9 contract
        # preserved (tiling DC-AE-specific, never global).
        if ratio != 32:
            return None

        # Vendor YAML (per-architecture) for tile parameters.
        vendor, arch = self._device_vendor_arch()
        if vendor is None or arch is None:
            return None
        from neurobrix.core.config import get_vae_decoder_config
        from neurobrix.core.components.handlers.dc_ae_tiled_decode import (
            should_tile_vae_decoder,
            latent_tile_params,
            tiled_decode,
        )
        cfg = get_vae_decoder_config(vendor, arch)
        if not should_tile_vae_decoder(
                latent.shape, ratio, cfg["tile_above_image_dim"]):
            return None  # below threshold → single-shot path

        # Tile parameter conversion to latent space.
        tile_latent_min, tile_latent_stride = latent_tile_params(
            cfg["tile_sample_min"], cfg["tile_sample_stride"], ratio)

        def _decode_tile(tile_latent):
            tile_inputs = dict(comp_inputs)
            tile_inputs[latent_key] = tile_latent
            result = executor.run(tile_inputs)
            if isinstance(result, dict):
                # Component returns a single 4D output for VAE; pick it.
                # `output_0` is the canonical name; fall back to first.
        ...
        return tiled_decode(
            latent, _decode_tile,
            tile_latent_min=tile_latent_min,
            tile_latent_stride=tile_latent_stride,
            spatial_compression_ratio=ratio,
        )

    def _device_vendor_arch(self) -> tuple:
        """Read (vendor, architecture) from the Prism plan's device
        spec. Used by Layer 9 VAE tiling to look up per-arch tile
        parameters in the vendor YAML. Returns `(None, None)` if the
        plan doesn't expose device info — caller falls through to
        single-shot.
        """
        plan = self.plan
        if plan is None:
            return None, None
        # Plan-side device list — Prism's PrismProfile.devices.
        profile = getattr(plan, "profile", None)
        if profile is None:
            # Fallback: per-component allocation may carry vendor/arch.
            comps = getattr(plan, "components", None) or {}
            for alloc in comps.values():
                v = getattr(alloc, "vendor", None)
                a = getattr(alloc, "architecture", None)
                if v and a:
                    return v, a
            return None, None
        ...
        vendor = brand.value.lower() if brand is not None and hasattr(brand, "value") else None
