...
def estimate_op_workspace_bytes(
    ...
    if cl in ("convolution", "conv2d", "conv1d", "_convolution"):
        ...
        if not input_shapes or not output_shapes:
            return 0
        in_shape = input_shapes[0]   # [N, in_c, in_h, in_w]
        weight_shape = input_shapes[1] if len(input_shapes) > 1 else None
        out_shape = output_shapes[0]  # [N, out_c, out_h, out_w]
        if len(in_shape) < 4 or len(out_shape) < 4 or weight_shape is None or len(weight_shape) < 4:
            return 0
        n, in_c, _, _ = in_shape
        kh, kw = weight_shape[2], weight_shape[3]
        out_h, out_w = out_shape[2], out_shape[3]
        im2col_bytes = n * out_h * out_w * in_c * kh * kw * dtype_bytes
