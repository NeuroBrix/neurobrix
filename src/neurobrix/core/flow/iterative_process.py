...
class IterativeProcessHandler(FlowHandler):
    ...
    def _execute_main_loop(
        ...
        # FLUX-video (Open-Sora) declares a non-standard state_variable (global.img)
        ...
        if self._flux_video:
            try:
                ...
                _st = None
            if _st is None:
                _latent = self.ctx.variable_resolver.get("global.latents")
                self.ctx.variable_resolver.set(state_key, _latent)
        ...
        for step_idx, timestep in enumerate(iterator):
            ...
            for comp_name in step_components:
                ...
                if output is not None:
                    ...
                    _dd_step = int(_os.environ.get("NBX_DUMP_DIT_STEP", "0"))
                    if _dd and step_idx == _dd_step and self._is_loop_component(comp_name):
    ...
    def _execute_post_loop(self, post_loop: List[str], loop_components: List[str]) -> None:
        ...
        if state_key and self._packing_info is not None:
            ...
            if current_state is not None and isinstance(current_state, torch.Tensor) and current_state.dim() == 3:
                ...
                if pi.get('ndim') == 5:
                    # FLUX-video: 3D packed -> 5D [B, C, T, H, W]
    ...
    @staticmethod
    def _pack_latents_5d(latents: torch.Tensor) -> torch.Tensor:
        """Pack a 5D video latent to 3D for FLUX-style video transformers.

        [B, C, T, H, W] -> [B, T*(H/2)*(W/2), C*4]

        Matches the Open-Sora-v2 vendor `pack`:
            rearrange("b c t (h ph) (w pw) -> b (t h w) (c ph pw)", ph=2, pw=2)
        Token order (t h w) matches the FLUX img_ids grid; channel block (c ph pw).
        H, W are the LATENT spatial dims.
        """
        b, c, t, h, w = latents.shape
        latents = latents.view(b, c, t, h // 2, 2, w // 2, 2)
        latents = latents.permute(0, 2, 3, 5, 1, 4, 6)  # b t (h/2) (w/2) c ph pw
        latents = latents.reshape(b, t * (h // 2) * (w // 2), c * 4)
        return latents

    @staticmethod
    def _unpack_latents_5d(
        latents: torch.Tensor, frames: int, height: int, width: int, channels: int
    ) -> torch.Tensor:
        ...
        latents = latents.reshape(b, channels, frames, height, width)
        return latents
    ...
    def _get_component_timestep_scale(self, comp_name: str) -> float:
        ...
        if (self.ctx.pkg.manifest.get("family") == "video"
            ...
            # Data-driven discriminator: some flow-match denoisers scale the
            # timestep INTERNALLY (FLUX/MMDiT `t = time_factor * t`, captured as
            # an in-graph `mul(input::timesteps, ~num_train)`). Those EXPECT the
            # scheduler's raw [0,1] sigmas — applying the [0,num_train] scale here
            # too would DOUBLE-scale (sigma*num_train then *time_factor in-graph),
            # blowing past fp16 range -> inf -> cos/sin NaN -> dead latent
            # (Open-Sora-v2). Mochi-style denoisers have NO in-graph scaling and
            # genuinely need [0,num_train]. Detect per component, never branch on
            # model name.
    ...
    def _graph_scales_timestep_internally(self, comp_name: str, num_train: float) -> bool:
        ...
        Reads the captured DAG (executor._dag if loaded, else the component's
        graph.json in the runtime cache). Result cached per component. Pure graph
        inspection — no execution, no model-name branch. See
