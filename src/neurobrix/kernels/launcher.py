...
class ScreenedBuffer(NamedTuple):
    ...
    def element_bytes(self) -> int:
        """The bytes of the tensor's own elements — what a snapshot holds and a reference is
        compared with (the extent `nbytes` for a contiguous buffer, shape x itemsize for a view)."""
        if self.view is None:
            return self.nbytes
        _lo, shape, _strides, itemsize = self.view
        n = 1
        for d in shape:
            n *= int(d)
