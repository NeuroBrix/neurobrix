# Reduce Mean - Mean reduction along axis
...
@triton.jit
def _reduce_mean_1d_kernel(
    in_ptr,
    out_ptr,
    n_elements,
    BLOCK_SIZE: tl.constexpr,
):
    """Reduce entire 1D tensor to mean - partial sum per block."""
    pid = tl.program_id(0)
...
def _reduce_mean_axis_kernel(
    ...
    """Mean reduction along axis - pure Triton."""
    pid = tl.program_id(0)
    ...
    # Divide by reduce_size for mean
    mean_val = acc / reduce_size

    out_offset = outer_idx * inner_size + inner_idx
    tl.store(out_ptr + out_offset + tl.arange(0, 1), mean_val)
...
def _reduce_mean_impl(
    x: torch.Tensor,
    dim: Optional[List[int]] = None,
    keepdim: bool = False
) -> torch.Tensor:
    """Mean reduction using pure Triton - NO PyTorch fallback."""
    ...
    if len(dim) > 1:
        ...
        res = x
        for d in dim:
            res = _reduce_mean_impl(res, dim=[d], keepdim=keepdim)
        return res
...
@register_kernel(family="video", vendor="nvidia", tier="common", op_name="reduce_mean")
def reduce_mean_kernel(
    ...
    """
    Mean reduction using pure Triton.
    ...
    if isinstance(dim, int):
        dim = [dim]
    return _reduce_mean_impl(x, dim, keepdim)
