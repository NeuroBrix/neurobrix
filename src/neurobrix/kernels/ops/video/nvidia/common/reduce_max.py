# Reduce Max - Maximum reduction along axis
...
@triton.jit
def _reduce_max_1d_kernel(
    in_ptr,
    out_ptr,
    n_elements,
    BLOCK_SIZE: tl.constexpr,
):
    """Reduce entire tensor to max value - partial max per block."""
    pid = tl.program_id(0)
    ...
    # Atomic max to output
    tl.atomic_max(out_ptr, block_max)


@triton.jit
def _reduce_max_axis_kernel(
    in_ptr,
    out_ptr,
    outer_size,
    reduce_size,
    ...
    """Max reduction along axis - pure Triton."""
    pid = tl.program_id(0)
    ...
    tl.store(out_ptr + out_offset + tl.arange(0, 1), max_val)
...
def _reduce_max_impl(
    x: torch.Tensor,
    dim: Optional[List[int]] = None,
    keepdim: bool = False
) -> torch.Tensor:
    """Max reduction using pure Triton - NO PyTorch fallback."""
    ...
    if len(dim) > 1:
        ...
        res = x
        for d in dim:
            res = _reduce_max_impl(res, dim=[d], keepdim=keepdim)
        return res
...
@register_kernel(family="video", vendor="nvidia", tier="common", op_name="reduce_max")
def reduce_max_kernel(
    ...
    """
    Max reduction using pure Triton.
    ...
    if isinstance(dim, int):
        dim = [dim]
    return _reduce_max_impl(x, dim, keepdim)
