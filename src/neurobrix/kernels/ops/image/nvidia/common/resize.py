...
def _resize_nearest_kernel(
    ...
    # Map to input coordinates (nearest with centering)
    h_in = ((h_out.to(tl.float32) + 0.5) / scale_h).to(tl.int32)
    w_in = ((w_out.to(tl.float32) + 0.5) / scale_w).to(tl.int32)
