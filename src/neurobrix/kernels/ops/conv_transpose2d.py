...
def conv_transpose2d_kernel(
    ...
    weight_ptr,      # (C_in, C_out, KH, KW) contiguous
    ...
    For each output pixel (oh, ow), it iterates over all (c_in, kh, kw) and
    checks if the corresponding input pixel exists (integer division constraint).
    """
    # pid_0 = n * C_out + co
    ...
    for ci in range(C_in):
        for kh in range(KH):
            for kw in range(KW):
                # Inverse mapping: ih = (oh + pad_h - kh) / stride_h
                ih_num = oh + pad_h - kh
                iw_num = ow + pad_w - kw
                ...
                w_val = tl.load(weight_ptr + w_offset, mask=valid, other=0.0)
