...
class TritonIterativeProcessHandler:
    ...
    def _execute_main_loop(
        ...
        for step_idx, timestep in enumerate(iterator):
            ...
            # Dual-denoiser: run ONLY the active expert this step (high-noise
            # for t >= boundary, low-noise for t < boundary); non-denoiser loop
            # components (if any) always run. Single-denoiser models: unchanged.
            step_components = components
            if dual is not None:
                _t = (timestep.item() if isinstance(timestep, NBXTensor)
                      else float(timestep))
                _active = dual["high"] if _t >= dual["boundary"] else dual["low"]
                # Expert lifecycle (R30 mirror of core): the boundary
                # transition is ONE-WAY, so evict the retired high-noise
                # expert the moment the low expert takes over — the two 14B
                # experts share shard GPUs under pipeline_parallel and their
                # co-residency during the low stage OOMed the triton arena
                # (SDPA block 36 on the shared tail GPU). Fires once.
    ...
    def _setup_step_cache(self, components, dual, num_steps):
        ...
        (triton/flow/step_cache.py, also consumed by the image_gen
        leg)."""
        if dual is not None:
            return None
    ...
    def _setup_dual_denoiser(self, components: List[str], driver: Any) -> Optional[dict]:
        ...
        R30 mirror of core/flow/iterative_process.py._setup_dual_denoiser —
        pure metadata reads (defaults / topology roles / driver), zero torch.

        boundary_ratio (defaults.json, from the registry dual-expert config)
        gives boundary_timestep = ratio * num_train_timesteps. The HIGH-noise
        expert runs for timesteps >= boundary, the LOW-noise expert for
        timesteps < boundary. Expert ordering prefers the component role
        (denoiser = high-noise, denoiser_low_noise = low-noise); falls back to
        topology loop order. GATED: None unless boundary_ratio is set AND
