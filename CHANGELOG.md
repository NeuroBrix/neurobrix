...
- `neurobrix autotune status` (and its `--json` record, `proofs_by_backend`) counts the certified directory's proofs by the code generator they were made with (`triton <version>`, plus the backend name when not cuda): a setting is proven for one generator, and a Triton upgrade re-proves the directory before it is switched.
...
- `neurobrix autotune certify --reprove-generator` re-proves every certified setting whose proof was made under another code generator (Triton version) than the running one; with `--only-missing` a setting proven under the running generator is left alone. A Triton upgrade re-proves the directory this way before it is switched.
