...
### Fixed
- Prism sizes a component's activations at the dtype they will actually hold: a component on the conservative precision path (no calibration record for its graph, or a record measured "conservative" on this hardware) stores its matmul results in fp32, so its activations are estimated in fp32 while its weights keep their dtype — read from the precision contract's own decision (Mochi's VAE decoder was planned at half of what it runs).
