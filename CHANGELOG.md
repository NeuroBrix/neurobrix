...
- **NBXTensor expand views (`stride=0` dimensions) on CPU**: `to_cuda()` and `_transfer_tensor` both use `tensor._nbytes = numel × elem_size` as the memcpy size, but an expanded view reports `numel` in the expanded shape while its backing storage holds only the unexpanded elements. A CPU-side expand of a `(1, K)` bias to `(B, T, K)` followed by transfer would over-read the numpy buffer. Latent, separate from the bytes-vs-strides fix above; not exercised under zero3 today because Qwen3/DeepSeek masks and biases stay GPU-resident (masks are weightless ops routed to exec_device by `override_weightless_op_devices`; these LLMs have no linear biases). Will surface if a future zero3 model offloads an expand-origin tensor — fix would materialise the view before transfer or track backing nbytes separately from view nbytes. Reproducer walkthrough in `tests/scratch/divergence_inv/DIVERGENCE_REPORT.md` §"Open questions / possible expansions".
