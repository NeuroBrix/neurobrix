...
- The serving daemon's live stream carries the text: for a family whose answer is text, every per-token event also carries `text` (the delta to append, decoded by the engine's own tokenizer, a split multi-byte character held until complete) and `rewind` (characters to take back when a later token rewrites what was emitted) — a client shows text as it arrives and ships no tokenizer.
- Windows: a daemon instance named by `NBX_SOCKET_PATH` takes a loopback port the OS hands out at bind and records it in `<stem>.port`, where its clients and `info --json` read it; the default instance keeps the fixed port. Two instances no longer collide on one port.
- `neurobrix import --json` reports the import as it happens: one JSON line per phase (model info, licence, download with the real byte counts, downloaded, extracting, installed, done or error), human lines on stderr; under `--json` the licence is an explicit `--accept-license`, never a prompt. A model becomes visible under its name only after its extraction succeeded (it extracts beside it and is renamed in one motion); a failed extraction leaves nothing behind. `neurobrix remove --json` answers with one record of what it removed.
- A compiled-only install (no Triton wheel) starts again: the kernel package no longer fails at import when the launcher finds no Triton to route (a regression of the launcher seam, `c8ed017`, seen by the compiled-mode-needs-no-triton test).
- Triton engine: a component's device arenas (the blocks its weights live in) survive the two dictionary rewrites that used to drop their last reference — the weight-key re-binding (which now keeps every key it does not name) and the second load a declared MoE fusion makes for the weights it adds (whose arenas now JOIN the first load's instead of replacing them, even when that second load put nothing on a device). Ming-Lite-Omni on the Triton engine ran into a freed arena at its first embedding and was refused by the launcher's ownership door; it runs. The malloc trace (`NBX_MALLOC_TRACE`) now names, on every free, the engine frames that freed the block.
- `neurobrix autotune status` (and its `--json` record, `proofs_by_backend`) counts the certified directory's proofs by the code generator they were made with (`triton <version>`, plus the backend name when not cuda): a setting stays correct under any generator (the oracle proved the source, not the compiler); what a Triton upgrade may age is its rank as the fastest, by a few percent, so the directory is re-proven under the new generator as an optimisation pass — incremental, checkpointed, invisible to a request.
- The Triton engine's allocator opens the CUDA runtime library the environment's torch wheel ships (`nvidia/cuda_runtime/lib`) before any the system offers — one process, one runtime; beside a 12.2 system toolkit, torch 2.14's libraries bound an older runtime than they were built on and failed to import.
- `neurobrix autotune certify --reprove-generator` re-proves every certified setting whose proof was made under another code generator (Triton version) than the running one; a setting proven under the running generator is left alone. The old proofs stay valid and served meanwhile: the pass optimises a rank, it repairs nothing.
...
- README: "Volta (V100) and the PyTorch ecosystem" — the PyTorch branch on a V100 needs a `cu126` wheel (2.10.0 and 2.14.0 measured through `tools/stack_door.py`; cu128/cu129/cu130 and the PyPI 2.14.0 do not serve it); the torch-free Triton branch has no such limit (Triton 3.8.0's ptxas still compiles sm_70) — the product reason for R33, said for a user.
- A symbol no fed input binds no longer falls back to its trace value in silence: the fallback is said once per symbol on the run's output (`[SymShape] FALLBACK …`), and `NBX_STRICT_SYMBOLS=1` refuses it by name — a dimension frozen at the trace is a defect, not a limit.
- The Triton engine runs a seedless request under the declared default seed, as the PyTorch branch already did: the seed slot the CLI writes as empty no longer shadows the default, and the default is read from the merged defaults (family, generation type, container) instead of the container alone. Two runs of a video request without `--seed` drew different initial noise.
- Text-to-speech refuses, by name, on BOTH engines, a voice that speaks a language the container's embedded phoneme lexicon does not: reading French text with American-English rules produced fluent speech saying other words, which no level, duration or silence check could see. The refusal names the voice, both languages, and the two ways out (ask for a voice of the lexicon's language, or rebuild the container with a lexicon for that language). The phonemiser's log also marks a quoted prompt as cut instead of appearing to have dropped text.
- An int4-encoded weight can move itself: the handle now answers `to_cuda`, `to_cuda_async` and `to_cpu` like a dense weight, moving its three parts together. A plan that streams or offloads such a weight (a quantised 30B model on one card) failed at the first move.
- A video that carries fewer frames than the request asked for says so, with both counts: a model's VAE represents frames on a temporal grid, so a count that is not on it comes back as the nearest one below — correctly, and until now silently.
...
### Added
- When a reference answer is available, kernel tuning stops seating a configuration that
  reference contradicts, whatever the other candidates agree on. Agreement among candidates
  is a vote, and a vote can be unanimously wrong — which matters most on hardware whose
  settings nobody has certified yet, where the tuner explores a space nobody has seen.
- The engine asks the card itself how much shared memory a kernel block may use, instead of
  looking the number up in a table of the cards it ships profiles for. A tile is offered only
  if the hardware says it can hold it, and where the hardware does not answer, nothing is
  pruned — a guessed budget deletes configurations that work.
...
### Fixed
- The Triton engine's `floor`, `ceil`, `round` and `remainder` kernels refused half-precision inputs
  (Triton's rounding functions take fp32/fp64 only: `Expected dtype ['fp32', 'fp64'] but got fp16`
  — every `--triton` run of the RNNT speech model stopped at its first `floor`, every one of the
...
- **Conditional-flow-matching voice synthesis for multimodal builds**
  ...
  speech-to-text on every mode's WAV. Adds the `NBX_SPEECH_DUMP`
  diagnostic (default-off) capturing the leg's bisection points
  (span ids, speech codes, mel, waveform) on both engines.
...
- **ATen alias ops in Triton modes**: numpy-style ATen aliases
  ...
  alias exactly like its canonical op.
...
## 2026-05-10 — P-SANA-4KPX-RUNTIME POINT 7 full closure (full pipeline + anti-regression)
...
[Unreleased]: https://github.com/NeuroBrix/neurobrix/compare/v0.5.2...main
[0.5.2]: https://github.com/NeuroBrix/neurobrix/compare/v0.5.1...v0.5.2
[0.5.1]: https://github.com/NeuroBrix/neurobrix/compare/v0.5.0...v0.5.1
[0.5.0]: https://github.com/NeuroBrix/neurobrix/compare/v0.4.1...v0.5.0
