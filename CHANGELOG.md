...
- Text-to-speech refuses, by name, a voice that speaks a language the container's embedded phoneme lexicon does not: reading French text with American-English rules produced fluent speech saying other words, which no level, duration or silence check could see. The refusal names the voice, both languages, and the two ways out (ask for a voice of the lexicon's language, or rebuild the container with a lexicon for that language). The phonemiser's log also marks a quoted prompt as cut instead of appearing to have dropped text.
