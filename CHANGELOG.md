...
- Triton engine: a component whose weight keys are re-bound to the graph's names at load (a wrapper-hierarchy difference, e.g. Ming-Lite-Omni's language model) no longer loses its device arenas in the re-binding — the arenas were freed under the live weights and the first kernel met a freed address, refused by the launcher; the re-binding now keeps every key it does not name.
