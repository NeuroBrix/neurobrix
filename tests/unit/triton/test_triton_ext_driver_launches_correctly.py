...
def test_scalars_arrive_whole_and_the_host_sees_the_writes():
    ...
    # rather than inherit it — and `_kernel()[grid](...)` CANNOT make it true.
    #
    # `install()` patches `JITFunction.__getitem__`, but the patch asks
    # `_is_ours(jit_fn)` first, and that reads the kernel's own `__module__`: a
    # kernel defined HERE belongs to `tests.unit.triton...`, which is foreign to
    # the engine by design. The seam hands a foreign kernel back to Triton's own
    # path — deliberately, so that torch's in-tree Triton kernels are not
    # refused at this engine's ownership rule. Measured 2026-09-17: through
    # `L.launch` the same kernel runs; through `kernel[grid](...)` it reaches
    # triton-ext with NBXTensors and dies "expected MetalBuffer ... got
    # NBXTensor".
    #
    # So this file was never exercising the launcher at all. It passed before
    # only because the archived fork's driver tolerated Triton's own path. The
    # launch below goes through `launch()` explicitly, which is what a test of
    # OUR launch ABI has to do when its kernel is not the engine's.
    ...
    launcher.launch(_kernel(), (triton.cdiv(n, BLOCK),), x, y, o, n, alpha, BLOCK=BLOCK)
