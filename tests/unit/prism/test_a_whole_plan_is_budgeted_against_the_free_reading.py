...
def test_the_effective_capacity_is_bounded_by_the_readings_rung():
    ...
    assert unbounded == min(32000.0 - s._margin_mb(32000.0), memory_ladder_rung_mb(32000.0))
    # a neighbour holds 20 GB: the reading is 12 000 MB, its rung 12 288 > reading?
    # no — rounding DOWN: 12 000 sits on the 8 192 rung
    dev.external_used_mb = 20000.0          # the machine's figure, not the plan's
    assert dev.free_mb == 12000.0
    assert s._effective_capacity_mb(dev) == 8 * 1024
    # 17 237 MB whole does not fit 8 192; it fitted 28 000-odd before
