"""Decoder graphs the KV tests share.

They live here, not in one test file imported by another: that import needed `tests` to be a
package, `tests/*` is ignored by the repository, and the marker that would make it work can
never be committed — so the whole directory failed to COLLECT, which reads exactly like a
directory that passes. Thirty tests were dark for that reason.
"""
...
@pytest.fixture
def dag_no_positions():
    """The same decoder with its positional arange removed: nothing the cache could offset."""
    d = _dag()
    d["ops"].pop("aten.arange::0", None)
    return d
