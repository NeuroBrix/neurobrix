...
def _entry_gen(cls, ver, config="A"):
    """An entry whose proof names a generator and a memory class."""
    return _entry(cls, ver, config)


def test_a_stale_snapshot_may_not_write_over_a_newer_proof(tmp_path):
    """The hazard a SEEDED side tree creates, closed before the two 32 GB trees were merged.

    A freed card is given its class-mate's remaining work by copying the tree that class-mate is
    still filling, so `--reprove-generator` skips what is already ranked. The copy is a SNAPSHOT:
    every key the live tree re-proves after the seed is older in the copy. Merging the copy back
    afterwards would write the older proof over the newer one, and `moved` would count it as a
    success — nothing else looks.

    Injection: the generator comparison removed → the 3.6.0 proof lands and this fails.
    """
    src, dst = tmp_path / "src", tmp_path / "dst"
    _file(src / "k.fp32.json", {"key1": _entry_gen(32, "3.6.0", "STALE")})
    _file(dst / "k.fp32.json", {"key1": _entry_gen(32, "3.8.0", "FRESH")})
    rec = _run(src, dst, 32)
    assert rec["files"][0]["kept_newer_in_destination"] == 1, rec["files"][0]
    assert rec["files"][0]["moved"] == 0
    out = json.loads((dst / "k.fp32.json").read_text())["entries"]["key1"]
    assert out["config"]["BLOCK_M"] == "FRESH", "a newer proof was overwritten by a staler one"


def test_the_same_generator_still_moves(tmp_path):
    """Two valid proofs of one key under one generator: either may win, and refusing both would
    make the second source's work unmergeable."""
    src, dst = tmp_path / "src", tmp_path / "dst"
    _file(src / "k.fp32.json", {"key1": _entry_gen(32, "3.8.0", "FROM_SRC")})
    _file(dst / "k.fp32.json", {"key1": _entry_gen(32, "3.8.0", "IN_DST")})
    rec = _run(src, dst, 32)
    assert rec["files"][0]["moved"] == 1 and rec["files"][0]["kept_newer_in_destination"] == 0


def test_a_newer_proof_still_lands_on_an_older_one(tmp_path):
    """The direction the merge exists for."""
    src, dst = tmp_path / "src", tmp_path / "dst"
    _file(src / "k.fp32.json", {"key1": _entry_gen(32, "3.8.0", "NEW")})
    _file(dst / "k.fp32.json", {"key1": _entry_gen(32, "3.6.0", "OLD")})
    rec = _run(src, dst, 32)
    assert rec["files"][0]["moved"] == 1
    out = json.loads((dst / "k.fp32.json").read_text())["entries"]["key1"]
    assert out["config"]["BLOCK_M"] == "NEW"
...
def test_the_other_class_slot_is_compared_on_its_own(tmp_path):
    """The destination's PRIMARY is 16 GB and its 32 GB variant is what this merge touches, so
    the comparison must read the variant and not the primary's generator."""
    src, dst = tmp_path / "src", tmp_path / "dst"
    _file(src / "k.fp32.json", {"key1": _entry_gen(32, "3.6.0", "STALE32")})
    d = _entry_gen(16, "3.6.0", "PRIMARY16")
    d["variants"] = {"32g": _entry_gen(32, "3.8.0", "FRESH32")}
    _file(dst / "k.fp32.json", {"key1": d})
    rec = _run(src, dst, 32)
    assert rec["files"][0]["kept_newer_in_destination"] == 1, rec["files"][0]
    out = json.loads((dst / "k.fp32.json").read_text())["entries"]["key1"]
    assert out["variants"]["32g"]["config"]["BLOCK_M"] == "FRESH32"
    assert out["config"]["BLOCK_M"] == "PRIMARY16", "the other class's primary must not move"
