...
def speaker_the_artefact_requires(model: str) -> str:
    """The voice to ask for, or "" where the artefact needs none — the one brick
    `tools/artefact_voice.py` (moved there 2026-09-14 so the precision campaign
    composes the same request as these cells)."""
    import sys as _sys
    _sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "tools"))
    from artefact_voice import speaker_the_artefact_requires as _brick
    return _brick(model, CACHE_ROOT)
